...
def remove_dup_inplace(ss):
    ...
    for i in range(1, len(S)):
        ...
        # for each char compare every el before tail
        # to see if this new el is a duplicate
        while j >= 0 and S[i] != S[j]:
            j -= 1
        if j < 0: # ith char not a duplicate
            # add this char to the tail and advance the tail
            S[tail] = S[i]
            tail += 1
    ...
    if tail < len(S): # tail equals to length if no duplicate
        S[tail] = '0'
    return ''.join(S)
